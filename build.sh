...
if [ $BUILD_FREEBSD == 1 ]; then
	for arch in "i386" "amd64"; do
		...
		# Copy PDF tools and data from graphics/poppler
		cp "$CALLDIR/pdftools/pdftotext-freebsd-$arch" "$APPDIR/pdftotext"
		cp "$CALLDIR/pdftools/pdfinfo-freebsd-$arch" "$APPDIR/pdfinfo"
	        cp -R "$CALLDIR/pdftools/poppler-data" "$APPDIR/"
